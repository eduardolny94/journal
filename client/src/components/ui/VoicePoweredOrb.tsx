// Orbe WebGL (ogl) con la paleta Global Traders FX: verde neón sobre negro.
// Adaptado del componente "voice-powered-orb": rota suavemente en reposo y, si se activa el
// control por voz, reacciona al micrófono. En el login se usa sin micrófono.
...
interface VoicePoweredOrbProps {
  className?: string;
  /** Rotación del tono en grados (0 = verde de marca). */
  hue?: number;
  enableVoiceControl?: boolean;
  voiceSensitivity?: number;
  maxRotationSpeed?: number;
  maxHoverIntensity?: number;
  /** Velocidad de giro en reposo (sin voz). */
...
const FRAG = /* glsl */ `
  ...
  // Paleta Global Traders FX
  const vec3 baseColor1 = vec3(0.086, 0.961, 0.478); // verde neón #16f57a
  const vec3 baseColor2 = vec3(0.640, 1.000, 0.820); // menta luminosa
  const vec3 baseColor3 = vec3(0.020, 0.180, 0.095); // verde profundo
...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
    ...
    try {
      ...
      const update = (t: number) => {
        ...
        if (enableVoiceControl && micReady) {
          ...
          if (voiceLevel > 0.05) currentRot += dt * (baseRotationSpeed + voiceLevel * maxRotationSpeed * 2.0);
      ...
      console.error('No se pudo iniciar el orbe WebGL:', error);
    }
  ...
  }, [hue, enableVoiceControl, voiceSensitivity, maxRotationSpeed, maxHoverIntensity, idleRotationSpeed]);
